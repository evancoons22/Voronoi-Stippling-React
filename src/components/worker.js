    ...
    for (let k = 0; k < 80; ++k) {
      ...
      for (let y = 0, i = 0; y < height; ++y) {
        for (let x = 0; x < width; ++x) {
          const w = data[y * width + x]; // this goes y rows down and x over. to get the current point.
      ...
      for (let i = 0; i < n; ++i) {
        ...
        const x1 = s[i] ? c[i * 2] / s[i] : x0, y1 = s[i] ? c[i * 2 + 1] / s[i] : y0; // if s[i] this is computed. 
